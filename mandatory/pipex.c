...
/*   Updated: 2025/03/13 22:36:36 by yhajji           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv, char **ev)
{
	...
	if (argc == 5)
	{
		...
		pro_id1 = fork();
		if (pro_id1 == -1)
			ft_print_error("error in fork", fds);
		if (pro_id1 == 0)
			ft_process_child(argv, ev, fds);
		pro_id2 = fork();
		if (pro_id2 == -1)
			ft_print_error("error in the fork", fds);
		if (pro_id2 == 0)
			ft_process_child2(argv, ev, fds);
		(close(fds[0]), close(fds[1]));
		(waitpid(pro_id1, NULL, 0), waitpid(pro_id2, NULL, 0));
	}
	else
